...
pub struct Outcome {
    pub x1: i32,  // Cars rented at site #1
    pub x2: i32,  // Cars rented at site #2
    pub y1: i32,  // Cars returned at site #1
    pub y2: i32,  // Cars returned at site #2
}
...
impl Outcome {
    pub fn new(x1: i32, x2: i32) -> Outcome {
    ...
    fn locations_have_enough_cars(&self, s1: &State, a: i8) -> bool {
        ...
        let loc2_enough_cars = self.x2 <=
            s1.n2.checked_add_signed(a).expect("Overflow") as i32;
    ...
    pub fn solve(s1: &State, s2: &State, xt: u32, a: i8) -> Vec<Outcome> {
        ...
        for x in 0..(xt + 1) as i32 {
            let mut z = Outcome::new(x, (xt as i32 - x));
            if !z.locations_have_enough_cars(s1, a) {
                continue
            }
            z.y1 = s2.n1 as i32 - s1.n1 as i32 + z.x1 + a as i32;
            z.y2 = s2.n2 as i32 - s1.n2 as i32 + z.x2 - a as i32;
            if z.is_nonnegative() {
                outcomes.push(z);
            }
        }
...
mod tests {
    ...
    fn check_outcome(
        s1: &State, s2: &State, outcome: &Outcome, xt: u32, a: i8
    ) -> bool {
        let site1_valid = 
            s1.n1 as i32 - a as i32 - outcome.x1 + outcome.y1 == s2.n1 as i32;
        let site2_valid = 
            s1.n2 as i32 + a as i32 - outcome.x2 + outcome.y2 == s2.n2 as i32;
        let sufficient_cars =
            xt <= s1.n1.checked_add(s1.n2).expect("Overflow!") as u32;
    ...
    #[test]
    fn solve_for_outcome_nonzero_rentals() {
        // Arrange
    ...
    #[test]
    fn solve_for_outcome_nonzero_rentals_action() {
        // Arrange
        let s1 = State {n1: 4, n2: 2};
        let s2 = State {n1: 3, n2: 3};
        let xt: u32 = 2;
        let a: i8 = 1;
        // Act
